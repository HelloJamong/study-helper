...
- **AI 요약** — 변환된 텍스트를 Gemini, OpenAI 또는 OpenRouter API로 요약
...
| AI 요약 | Gemini / OpenAI / OpenRouter 중 선택해 API 키 설정 |
...
Gemini, OpenAI, OpenRouter 중 하나를 선택해 STT 결과를 자동 요약합니다. 설정 화면에서 에이전트를 먼저 선택한 뒤 해당 API 키와 모델을 입력합니다.

| 에이전트 | 특징 |
|----------|------|
| Gemini | 무료 티어 지원, 기본 권장 |
| OpenAI | 유료, 모델 목록에서 선택 |
| OpenRouter | OpenAI 호환 API로 다양한 모델 사용 가능, 일부 모델은 무료(`:free` 접미사) — 모델 ID를 직접 입력 |
